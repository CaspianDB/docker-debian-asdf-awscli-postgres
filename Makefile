DEBIAN_ASDF_TAG ?= $(shell docker run --rm gcr.io/go-containerregistry/crane ls caspiandb/debian-asdf | grep -P '^asdf-[0-9.]+-' | sort -V -r -t- | head -n1)
POSTGRES_RELEASE ?= $(shell cat .tool-versions | awk '$$1 == "postgres" { print $$2 }')
...
IMAGE_NAME ?= debian-asdf-awscli-postgres
...
push: ## Publish to container registry.
	...
	docker tag $(IMAGE_NAME) $(DOCKER_REPO):postgres-$(POSTGRES_RELEASE:v%=%)
	docker push $(DOCKER_REPO):postgres-$(POSTGRES_RELEASE:v%=%)
...
test: ## Test local image
	...
	docker run --rm -t ${IMAGE_NAME} psql --version | grep ^psql
